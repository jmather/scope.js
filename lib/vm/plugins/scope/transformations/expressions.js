...
ExpressionTransformer.prototype.resolve = function (input, output) {
    ...
    _.each(output.values, function(config, name) {
        ...
        output.values[name] = processExpression(name, config, resolver, output.expressionTemplates);
...
ExpressionValueNameResolver.prototype.resolve = function(name, value) {
    ...
    var parentParts = this.expressionName.split('.').slice(0, -1);
...
function processExpression(name, config, resolver, templates) {
    ...
    if (config.template) {
        if (templates[config.template] === undefined) {
            throw new Error('No expression template: ' + config.template);
        }
        config = _.defaults({}, config, templates[config.template]);
    } else {
