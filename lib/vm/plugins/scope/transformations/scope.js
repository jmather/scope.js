var _ = require('../../../../internal/underscore');
...
ScopeValueNameResolver.prototype.resolve = function(name, value) {
    ...
    var parentParts = this.scopeName.split('.').slice(0, -1);
    var parentName = parentParts.length > 0 ? parentParts.join('.') + '.' : '';

    var foundValues = [];

    var findValues = (value instanceof Array) ? value : [value];

    _.each(findValues, function(val) {
        if (this.values[parentName + val] !== undefined) {
            foundValues.push(parentName + val);
        } else if (this.values[val] !== undefined) {
            foundValues.push(val);
        } else {
            if (foundValues.length > 0) {
                throw new Error("Could not find a value with the name fragment of " + val);
            }
        }
    }.bind(this));
    ...
    return { value: (value instanceof Array) ? foundValues : foundValues[0] };
};
