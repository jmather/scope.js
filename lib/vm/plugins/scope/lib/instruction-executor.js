...
function getArgumentsForMethod(method) {
    ...
    if (match === null) {
        match = source.match(/^(?:async\s+)?([^=()\s,]+)\s*=>/);
        if (match !== null) {
            return [match[1]];
        }
        throw new Error('Unable to inspect instruction arguments: ' + source);
    }
    ...
    return match[1].split(',').map(function(argument) {
        var name = argument.trim().replace(/^\.\.\./, '').split('=')[0].trim();
        if (name.match(/^[a-zA-Z_$][a-zA-Z0-9_$]*$/) === null) {
            throw new Error('Instruction arguments must be named identifiers: ' + argument.trim());
        }
        return name;
    });
}
